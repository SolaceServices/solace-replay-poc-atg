...
for fname in $(ls $test_dir); do
   ...
   topic_t=$topic_prefix/$topic_p
   ts=$(date "+%s")
   outfile=$PWD/out/run-$me-$ts.out
   > $outfile || { echo Unable to create output file $outfile; exit 1; }
   echo "$file: Topic: $topic_t Num: $msg_num Rate: $msg_rate Size: $msg_size  Transport: $msg_trans Clients: $num_clients Threads: $num_threads CF: $conn_factory"
   ( $sdkperf_path/sdkperf_jms.sh -cip=$msg_ip -cu=$user@$vpn -cp=$pass -ptl=$topic_t -mn=$msg_num  -mt=$msg_transport -mr=$msg_rate -msx=$msg_size -cc=$num_clients -cpt=$num_threads $sdk_flags  > $outfile 2>&1 ) &
